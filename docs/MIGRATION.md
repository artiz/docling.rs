...
- **JSON** rebuilds docling's full `body`-tree-of-`$ref`s model from the `Node`
  ...
  the HTML backend's `html_tree.rs`): docling's items in creation order with
  their parents, children, layers, formatting and hyperlinks, serialized as
  they are — which is how the HTML JSON is structurally identical to
  upstream's (see the HTML row).
