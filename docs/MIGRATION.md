...
| LaTeX | 2 | **2** | 0 | JSON is the flat shape (Markdown: pylatexenc's chars-node paragraph split and the trailing blank `tabular` row ported) |
...
| LaTeX | `latex.rs` (scanner) | simple `.tex` **2/2 exact** vs docling 2.129 — the scanner now mirrors pylatexenc's node stream where it shows in the output: a chars node holding a paragraph break is split there with its first part stripped (so the space after a paragraph's last macro is lost, `italictext.`, as upstream loses it) and every later part is a paragraph of its own, and a `tabular` gets docling's trailing blank row (the newline around a closing `\hline` is a chars node that finishes one more row); multi-file arXiv projects convert too — 6 projects carry committed groundtruth and snapshot pinning (`tests/snapshots/latex`) |
