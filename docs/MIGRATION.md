...
| LaTeX | 2 | 1 | 0 | JSON is the flat shape; the trailing blank `tabular` row is ported, the one-word Markdown difference is deliberate — docling's `_process_chars_node` strips the leading space of the chars node that closes a paragraph (`italictext.`), reported upstream, we keep the space |
