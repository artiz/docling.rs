...
| AsciiDoc | 4 | **4** | **4** | — (JSON: docling's `parents`/`indents` tree — title root, headings at their level, nested `list` groups, body-level captions) |
| LaTeX | 2 | 1 | 1 | JSON: docling's body-flat tree — `title`/`text` for `\maketitle`, `section_header` levels, `paragraph` for the text a chars node's `\n\n` splits off vs `text` for buffered text, `formula` items, `list` groups whose items are `enumerated: false` with empty markers (an `enumerate` too), `tabular` cells without header roles incl. the trailing blank row; `example_01` differs by the same one word as its Markdown. The trailing blank `tabular` row is ported, the one-word Markdown difference is deliberate — docling's `_process_chars_node` strips the leading space of the chars node that closes a paragraph (`italictext.`), [docling#4339](https://github.com/docling-project/docling/issues/4339); we keep the space |
...
| USPTO | 9 | **9** | 1 | `pftaps` plain-text patents parse like docling's `PatentUsptoGrantAps` (Markdown and JSON tree exact); `tables_ipa20180000016` fails in upstream; the XML paths' JSON is the flat shape |
...
| LaTeX | `latex.rs` (scanner) | simple `.tex` **1/2 exact** (Markdown and JSON alike) vs docling 2.129, the other file one word off *on purpose*: docling's `_process_chars_node` splits the pylatexenc chars node that holds a paragraph break and `strip()`s the part before it, losing the space after the paragraph's last macro (`\textit{italic} text.` → `italictext.`, upstream's own groundtruth reads so) and cutting a sentence that continues with a macro after the break into two paragraphs — text-corrupting, so not reproduced ([docling#4339](https://github.com/docling-project/docling/issues/4339); the fixture will converge when docling fixes it). The JSON is docling's body-flat tree: `title` and author `text` from `\maketitle`, `section_header` levels per `_get_heading_level`, the `paragraph` label for text a chars node's `\n\n` split off (vs `text` for what went through the text buffer — a merged item keeps `text`), `formula` items for display math, `list` groups whose items are `enumerated: false` with empty markers, `tabular` cells with no header roles. A `tabular` does get docling's trailing blank row (the newline around a closing `\hline` is a chars node that finishes one more row); multi-file arXiv projects convert too — 6 projects carry committed groundtruth and snapshot pinning (`tests/snapshots/latex`) |
