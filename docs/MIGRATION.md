...
| Markdown | 10 | **10** | 5 | the five files with a raw HTML block: upstream re-converts those through its HTML backend over docling-core's HTML export, which has no counterpart here (the flat export stays) |
| DeepSeek-OCR Markdown | 3 | **3** | n/a | — (2.129's first-row-only `column_header` for HTML tables ported) |
| AsciiDoc | 4 | **4** | 0 | JSON is the flat shape (Markdown: 2.127+'s bold block titles and in-order sub-subsections/images ported) |
...
| ODF | 7 | **7** | **6** | `odf_presentation_02`'s JPEG keeps its own mimetype where PIL re-encodes to PNG |
...
| EBCDIC | 3 | **3** | **3** | — |
| DocLang (`.dclg`/`.dclx`) | 15 | **15** | 0 | JSON: `pages` for a bare `.dclg`, list group names, markers (Markdown: docling-core's dropped `<!-- missing-text -->` and hard-break rich-cell padding ported) |
