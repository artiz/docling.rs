...
| USPTO | 9 | 6 | 1 | `pftaps` plain-text patents parse like docling's `PatentUsptoGrantAps` (Markdown and JSON tree exact); two XML files differ (`pa20010031492` 14 lines, `ipa20110039701` 489); `tables_ipa20180000016` fails in upstream; the XML paths' JSON is the flat shape |
...
| USPTO | `uspto.rs` | **6/9 exact (7/9 whitespace-normalized) vs docling 2.129, JSON 1/9** — was 1/5; the legacy APS plain-text patent (`pftaps*.txt`) now goes through a port of docling's `PatentUsptoGrantAps` (title, `ABSTRACT`, `PAC` caption headings, paragraphs, `CLAIMS`; Markdown, JSON item tree and `.dclx` all exact), `pa20010031492`/`ipa20110039701` still differ, and `tables_ipa20180000016.xml` fails in upstream itself. Earlier: **1/5 exact (2/5 whitespace-normalized)** on the sources live docling converts — it errors on the other 5 (those are validated byte-exact via `.dclx`), and its APS-text *Markdown* export is empty where ours emits the text dump (the `.dclx` matches exactly — §5) |
