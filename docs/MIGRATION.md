...
|---|---|---|---|---|
| DOCX | 36 | **36** | **35** | `drawingml`'s JPEG keeps its own mimetype where PIL re-encodes to PNG |
...
| Markdown | 10 | **10** | 2 | flat-path bullets carry `marker: "-"` where docling writes `""` |
| DeepSeek-OCR Markdown | 3 | 2 | n/a | 2.129 renders the annotated table with a repeated header row per column-group line and no numeric alignment |
| AsciiDoc | 4 | 2 | 0 | 2.127+ bolds a block title (`**Procedure**`) and keeps a sub-subsection and its images in document order where we hoist them after the table; JSON is the flat shape |
| LaTeX | 2 | 0 | 0 | the known `italictext` run join |
| JATS | 6 | **6** | 0 | JSON is the flat shape (the `.txt` copy of `elife-56337` is plain text to us, JATS to docling — not counted) |
| USPTO | 9 | 5 | 0 | `pftaps` plain-text patents (docling's `.txt` variant) are not read; two XML files differ (`pa20010031492` 14 lines, `ipa20110039701` 489); `tables_ipa20180000016` fails in upstream; JSON is the flat shape |
| WebVTT | 4 | **4** | 0 | 2.129 writes a per-cue `source` (`kind: track`, start/end, voice) and `formatting` on styled cues — neither in our JSON |
| ODF | 7 | **7** | 0 | JSON is the flat shape |
| EPUB | 1 | 0 | **1** | the known 4-line HTML inline residual |
...
| EBCDIC | 3 | **3** | **3** | — |
| DocLang (`.dclg`/`.dclx`) | 15 | 13 | 0 | docling-core stopped writing `<!-- missing-text -->` for key-value items and pads a rich cell's inline text differently; JSON: `pages` for a bare `.dclg`, list group names, markers |
...
| WebVTT | `webvtt.rs` | **4/4 exact** (re-verified vs docling 2.129; JSON 0/4 — 2.129 writes a per-cue `source` provenance (`kind: track`, start/end time, voice) and `formatting` on styled cues, neither in our JSON yet); #366: karaoke cue timestamps (`<00:00:00.389>`) are stripped without splitting the span (docling-core#744), text after a multi-line voice span stays in reading order (docling#4105), bare CR / CRLF line terminators parse (docling-core#749, docling#4157) — pinned by unit tests |
...
| USPTO | `uspto.rs` | **5/9 exact (6/9 whitespace-normalized) vs docling 2.129** — was 1/5; `pftaps` plain-text patents (docling's `.txt` USPTO variant) are not read yet, `pa20010031492`/`ipa20110039701` still differ, and `tables_ipa20180000016.xml` fails in upstream itself. Earlier: **1/5 exact (2/5 whitespace-normalized)** on the sources live docling converts — it errors on the other 5 (those are validated byte-exact via `.dclx`), and its APS-text *Markdown* export is empty where ours emits the text dump (the `.dclx` matches exactly — §5) |
...
| DocLang (`.dclg`/`.dclx`) | `doclang.rs` (roxmltree) | **15/15 exact** vs live docling 2.126 (**13/15 vs 2.129**: docling-core no longer writes `<!-- missing-text -->` for key-value items and pads a rich cell's inline text differently — to port), measured reading the same archives back (`tests/data/doclang`); the inverse of the `.dclx` output serializer, incl. docling's round-trip losses (list-item formatting, hyperlink targets) |
