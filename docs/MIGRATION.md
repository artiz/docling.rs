...
| Table-cell repair API | `docling-core` `Table::{cell_text, set_cell_text, cell_bbox, set_cell_bbox, find_cell_by_bbox, update_cell_by_bbox}`, `DoclingDocument::tables[_mut]` (#238) | **docling.rs extension** (Python docling exposes `TableCell` objects; this is the Rust counterpart for post-extraction repair): the PDF TableFormer paths record per-cell page-point boxes (`Table::cell_boxes`, spans repeat the anchor), bbox lookup is best-IoU, and edits flow into every serializer since the text grid is the single source of truth. Wire formats are unchanged (no serializer reads the geometry); emitting cell bboxes in docling JSON is a follow-up |
