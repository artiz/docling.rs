...
`\hline` rules and captions, `figure` environments with a `% image`
placeholder and the picture classification as a `% annotation` comment,
`verbatim` code, `$$…$$` formulas, and LaTeX escaping of every special
character in text. Output is **byte-for-byte** with Python docling-core's
serializer on the declarative corpus (207/207 fixtures,
`crates/docling/tests/regression.rs`'s `.tex` expectations are generated by
the Python serializer over the same documents).
...
One deliberate deviation: upstream raises on a heading deeper than
`\subsubsection`; docling.rs degrades those to `\paragraph` /
`\subparagraph` instead of failing the conversion.
