...
// (download_dependencies.sh for local dev; the pre-rename ./models/ still
// counts), then a CORS-enabled Hugging
...
// profile is chosen.
const TF_DIRS = ["./.models/tableformer/", "./models/tableformer/", MODEL_BASE];
...
export function createOcr({ onStatus }) {
  ...
  async function loadLayout() {
    for (const [name, kind] of [
      ...
      let buf;
      try {
        ({ buf } = await fetchModel(name, ["./.models/", "./models/", MODEL_BASE], "layout model (first load only)"));
      } catch (e) {
